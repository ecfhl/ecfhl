...
<div class="shell">
  ...
  <section class="section">
    <div class="section-title"><h2>Individual awards</h2></div>
    <div class="grid-3">
      @foreach($awards as $a)
        @php
          $icon = match($a['id']) {
  ...
  <section class="section" style="padding-top:0">
    <div class="season-links-grid">
      ...
      <a class="card season-action-card" href="/draft?season={{ urlencode($season['season']) }}">
        <span class="subtle">Top 3 draft picks</span>
        <div class="top-picks-list">
          @forelse($topPicks as $i=>$pick)
            <div><b>{{ $i+1 }}.</b> {{ $pick['player'] ?? '—' }} <small>{{ $pick['team'] ?? '' }}</small></div>
          @empty
            <div class="subtle">No draft data</div>
          @endforelse
        </div>
