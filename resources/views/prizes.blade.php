    ...
    <div>
      ...
      <div class="table-card"><div class="table-scroll"><table class="data-table">
        ...
        <tbody>@foreach($totals as $r)<tr>
          <td><a href="/teams/{{ \Illuminate\Support\Str::slug($r['franchise_name']) }}"><strong>{{ $r['franchise_name'] }}</strong></a></td>
    ...
    <div>
      ...
      <div class="table-card"><div class="table-scroll"><table class="data-table awards-season-table">
        ...
        @foreach($awardsBySeason as $row)<tr>
          ...
          <td>@foreach($row['playoffs'] as $item)<div>{{ $item }}</div>@endforeach</td>
          <td>@forelse($row['regular'] as $item)<div>{{ $item }}</div>@empty<span class="subtle">—</span>@endforelse</td>
          <td>@foreach($row['individual'] as $item)<div>{{ $item }}</div>@endforeach</td>
        </tr>@endforeach
        </tbody>
      </table></div></div>
    </div>
