...
@foreach($teams as $t)<tr><td><a href="/teams/{{ IlluminateSupportStr::slug($t['team']) }}"><strong>{{ $t['team'] }}</strong></a></td><td class="num">{{ $t['fantrax_seasons'] ?? '—' }}</td><td class="num">{{ $t['titles'] ?? 0 }}</td><td class="num">{{ $t['finals'] ?? 0 }}</td><td class="num">{{ $t['h2h_first'] ?? 0 }}</td><td class="num">{{ ($t['w']??0).'-'.($t['l']??0).'-'.($t['t']??0) }}</td><td class="num">{{ isset($t['win_pct']) ? number_format($t['win_pct']*100,1).'%' : '—' }}</td></tr>@endforeach
